...
function renderTask(condition){
  ...
  document.getElementById("taskModeTitle").textContent = framework ? "Explore and compare at least two complete pathways" : "Examine the complete pathway presented";
  document.getElementById("taskFeatureCopy").textContent = framework
    ? "After completing a pathway through Impact, its Final Report becomes available. You can then ask a stakeholder persona questions; the persona responds from the perspective represented in that completed pathway."
  ...
  const items = framework ? [
    {title:"Complete two different pathways", detail:"Choose branches and follow at least two distinct pathways through the Impact phase."},
    ...
    {title:"Read both Final Reports", detail:"Review the Final Report for each of the two pathways you complete."},
    {title:"Ask a stakeholder a question", detail:"On a completed pathway, select a stakeholder persona whose responses reflect the position represented by that pathway, and ask at least one question."},
    {title:"Compare before continuing", detail:"Consider how the pathways differ in their development, constraints, and outcomes before finishing the case."},
  ] : [
